...
pub const Env = struct {
    ...
    fn init(alloc: std.mem.Allocator) !Env {
        return .{
    ...
    fn register(self: *Env, name: []const u8) !void {
    ...
    fn release(self: *Env, name: []const u8) void {
        if (self.bindings.getPtr(name)) |arr| {
            _ = arr.pop();
        }
    }

    fn get(self: *const Env, name: []const u8) ?[]const u8 {
...
fn renameLet(env: *Env, node: *Node) anyerror!void {
    ...
    try innerExpand(env, bindings.data.list.items[1]);
    ...
    while (i < bindings.data.list.items.len) : (i += 2) {
        ...
        try innerExpand(env, bindings.data.list.items[next_i]);
    }
...
fn innerExpand(env: *Env, node: *Node) anyerror!void {
    switch (node.data) {
        .id => |name| {
...
pub fn expand(state: *State, node: *Node) anyerror!void {
    var env = Env{
        .alloc = state.arena_alloc,
        .bindings = std.StringHashMap(std.ArrayList([]const u8)).init(state.arena_alloc),
    };
    try innerExpand(&env, node);
}
